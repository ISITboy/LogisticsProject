...
fun DialogForMembers(
    ...
    AlertDialog(
        ...
        text = {
            if (openDialog.value == DialogEvent.OpenConsumerDialog) {
                Column(
                    modifier = Modifier
                        .fillMaxWidth()
                        .fillMaxHeight(0.3f),
        ...
        confirmButton = {
            Button({
                confirmEvent(
                    ...
                    clickSaveShipper = {
                        if (typeEvent) {
                            event(MembersEvent.AddProducerItem(Producer(address = texts.first().first.value)))
                        } else event(MembersEvent.UpdateProducerItem(Producer(address = texts.first().first.value)))
                    },
                    clickSaveConsignee = {
                        if(typeEvent){
                            event(
                                MembersEvent.AddConsumerItem(
                                    Consumer(
